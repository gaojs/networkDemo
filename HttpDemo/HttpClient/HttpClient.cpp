	...
	int event_type, void* event_data)
{
	...
	case MG_EV_HTTP_REPLY:
	{
		...
		string rsp = string(hm->body.p, hm->body.len);
...
void HttpClient::SendReq(const string& url,
	ReqCallback req_callback)
{
	// 给回调函数赋值
	if(s_exit_flag)
	{
		...
		auto connection = mg_connect_http(&mgr, OnHttpEvent,
			url.c_str(), NULL, NULL);
